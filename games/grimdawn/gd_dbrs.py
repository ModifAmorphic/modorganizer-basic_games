...
class ModExtractor:
    ...
    def extract_grimdawn(self, callback: Callable[[ExtractStatus], None]) -> str:
        databases = [
            ...
            path.join(self._game_path, "survivalmode1", "database", "SurvivalMode1.arz"),
            path.join(self._game_path, "survivalmode2", "database", "SurvivalMode2.arz"),
            path.join(self._game_path, "gdx1", "database", "GDX1.arz"),
            path.join(self._game_path, "gdx2", "database", "GDX2.arz")
        ]
        t = threading.Thread(target=self.extract_databases, args=[databases, self.GameExtractPath, callback])        
        t.daemon = False
        t.start()
