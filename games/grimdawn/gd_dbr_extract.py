...
class ModExtractor(MultiStepTask):
    ...
    def _extract_databases(self, databases: list[Path], out_path: Path):
        ...
        for db in database_extracts:
            ...
            process.finished.connect(lambda ex_paths=extract_paths: self._on_extract_complete(exitCode, exitStatus, ex_paths, remaining_dbs, database_extracts))  # noqa # fmt: skip # type: ignore
