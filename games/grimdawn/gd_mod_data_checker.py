...
class GrimDawnModDataChecker(BasicModDataChecker):
    ...
    _fixable_paths: dict[re.Pattern[str], str] = {
        ...
        re.compile(
            r"^(?:.*/)?templates.arc$", re.IGNORECASE
        ): "mods/{{mod_folder}}/database",
        re.compile(r"^.*\.arc$", re.IGNORECASE): "mods/{{mod_folder}}/resources",
        re.compile(
            r"^(?:.*/)?video/(?!.*/).*$", re.IGNORECASE
        ): "mods/{{mod_folder}}/video",
        re.compile(
            r"(?:^|(?<=/))(text_[a-zA-Z]{2})(?=/)(.*)", re.IGNORECASE
        ): "settings",
        re.compile(r"(?:^|(?<=/))(ui)(?=/)(.*\.(tex|psd))", re.IGNORECASE): "settings",
        re.compile(
            r"(?:^|(?<=/))(fonts)(?=/)(.*\.(bmp|fnt|ttf|txt))", re.IGNORECASE
        ): "settings",
        ...
        for r in self._fixable_paths:
            ...
            if sresult:
                ...
                if fix_path == "settings":
                    ...
                    qDebug(
                        f'Joining "{fix_path}" path with filename "{filetree.name()}".'
                    )
    ...
    def pruneEmptyDirs(self, filetree: IFileTree, recursive: bool = False):
        ...
        qDebug(
            f"Pruning empty directories from path {filetree.path(self._REGEX_DIR_SEP)}. recursive={recursive}"
        )
        ...
        for mod_tree in self.getNextFileTree(tree, True):
            ...
            if mod_tree.entry.isDir() and mod_tree.entry.name() != "":
                qDebug(
                    f"Queuing Empty Directory {d_path} ({mod_tree.entry.name()}) for removal."
                )
        ...
        if recursive:
            if detaches:
                qDebug(
                    f"Last prune found {len(detaches)} empty directories. Continuing until no empty directories remain."
                )
    ...
    def dataLooksValid(self, filetree: IFileTree) -> ModDataChecker.CheckReturn:
        ...
        for modEntry in self.getNextFileTree(filetree):
            ...
            if modEntry.matchResult == MatchResult.UNKNOWN:
                ...
                qDebug(
                    f'dataLooksValid() Unknown file "{m_path}" found. Returning status {ModDataChecker.INVALID}'
                )
                return ModDataChecker.INVALID
            if modEntry.matchResult == MatchResult.FIXABLE:
                return_status = ModDataChecker.FIXABLE
            if (
                modEntry.matchResult == MatchResult.VALID
                and return_status == ModDataChecker.INVALID
            ):
    ...
    def fix(self, filetree: IFileTree) -> IFileTree:
        ...
        for m in moves:
            qDebug(
                f'Moving file "{m.entry.path(self._REGEX_DIR_SEP)}" to "{m.fixPath}".'
            )
